#!/usr/bin/env python3
"""
wiki_fts.py — BM25 全文搜索脚本 for 工作知识库
用法:
...
def build_index():
    """扫描 wiki/ 目录建立 BM25 索引"""
    os.makedirs(INDEX_DIR, exist_ok=True)
    docs = {}
    for dirpath, dirs, files in os.walk(os.path.join(WIKI_ROOT, "wiki")):
        for f in files:
...
if __name__ == "__main__":
    ...
    sub.add_parser("build", help="构建 BM25 索引")
    sub.add_parser("stats", help="索引状态")
    ...
    if not vars(args):
        ...
        search(" ".join(args.query), args.limit)
    elif hasattr(args, 'func'):
        {"build": build_index, "stats": stats}[args.func]()
    else:
        list(vars(args).keys())[0] + "()"
