# Everything Claude Code - GitHub 项目文档
...
**Everything Claude Code (ECC)** 是一个专为 AI 代理工具链设计的性能优化系统，由 Anthropic Hackathon 获奖者开发。该项目拥有 **14万+ stars**、**2.1万+ forks** 和 **170+ 贡献者**。
...
1. **跨平台支持**
   ...
   - Codex CLI/App
   - Gemini CLI
   - Antigravity / JoyCode / Qwen CLI

2. **组件架构**
   | 组件 | 数量 | 说明 |
   |------|------|------|
   | Agents | 53 | 子代理处理委托任务 |
   | Skills | 203 | 工作流技能（主要工作面） |
   | Commands | 69 | 斜杠命令入口 |
   | Hooks | 20+ | 工具事件钩子 |
   | Rules | 34 | 语言规则（common + 多语言） |
   | MCP Servers | 14 | 模型上下文协议服务器 |
...
- **v2.0.0-rc.1**: Dashboard GUI、运营工作流扩展、ECC 2.0 alpha
- **v1.9.0**: 选择性安装、6个新代理、12种语言生态
- **v1.8.0**: Hook 可靠性重构、NanoClaw v2
- **v1.7.0**: Codex 支持、演示文稿构建器
- **v1.6.0**: AgentShield 集成、GitHub Marketplace 上线

### 项目定位

> "不仅仅是配置，而是一套完整系统：技能、本能、内存优化、持续学习、安全扫描和研究优先开发。经过10个多月的实战验证。"
