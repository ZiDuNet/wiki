# AI-DLC (AI-Driven Development Life Cycle) - GitHub 项目文档
...
### 项目概述
AI-DLC（AI驱动开发生命周期）是 AWS Labs 推出的智能软件开发工作流框架，它能够适应您的需求、保持质量标准，并让您掌控整个开发过程。

### 核心特点

**三阶段自适应工作流：**
1. **🔵 启动阶段 (Inception Phase)** - 确定**构建什么**和**为什么构建**
   - 需求分析和验证
   - 用户故事创建
   - 应用设计和工作单元划分
   - 风险评估和复杂性评估

2. **🟢 构建阶段 (Construction Phase)** - 确定**如何构建**
   - 详细组件设计
   - 代码生成和实现
   - 构建配置和测试策略
   - 质量保证和验证

3. **🟡 运营阶段 (Operations Phase)** - 部署和监控（未来功能）
   - 部署自动化和基础设施
   - 监控和可观测性设置
   - 生产就绪性验证
...
| 特性 | 描述 |
|------|------|
| **自适应智能** | 仅执行对特定请求有价值的阶段 |
| **上下文感知** | 分析现有代码库和复杂性需求 |
| **基于风险** | 复杂变更获得全面处理，简单变更保持高效 |
| **问题驱动** | 通过文件中的结构化多选题提问，而非聊天 |
| **始终可控** | 审查执行计划并批准每个阶段 |
| **可扩展** | 可在核心工作流之上叠加自定义规则（安全、合规等）|
...
### 安装方式
1. 从 [Releases 页面](https://github.com/awslabs/aidlc-workflows/releases/latest) 下载最新版本
2. 解压 zip 文件，包含：
   - `aws-aidlc-rules/` — 核心 AI-DLC 工作流规则
   - `aws-aidlc-rule-details/` — 核心规则引用的详细规则
3. 根据不同 IDE 平台复制到相应目录

### 扩展系统
AI-DLC 支持扩展系统，可以在核心工作流之上叠加额外规则：
- **安全扩展** (`security/baseline/`)
- **测试扩展** (`testing/property-based/`)
- 支持自定义扩展（合规性、性能等）

### 核心原则
- **无重复** - 真相源只在一处
- **方法论优先** - AI-DLC 本质上是方法论，而非工具
- **可复现** - 规则清晰，不同模型产生相似结果
- **无关性** - 适用于任何 IDE、代理或模型
- **人在回路** - 关键决策需要用户明确确认
