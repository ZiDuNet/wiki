> 📎 来源: [AI末班车](https://mp.weixin.qq.com/s?__biz=MzkxNzE5Mjc0Ng==&mid=2247484411&idx=1&sn=e94ca257d693f77d0829b66be20b4b7f&chksm=c0fd33bb75d98aa8a9a4d16df952b4d6ba909e3e48529ca3a56a26ca3bc8a854153b318aa561&mpshare=1&scene=1&srcid=0511yI8TKQkJZkndi4bgSJnv&sharer_shareinfo=a18989804ae7930f26889110bdadfacd&sharer_shareinfo_first=a18989804ae7930f26889110bdadfacd) | 时间: 2026-05-10 19:08
...
。如果新的 val\_bpb 比之前最佳值更低，保留修改；如果更差，
...
里有一条关键指令："**永远不要停下来。实验循环一旦开始，不要暂停来问人类是否应该继续。**"
...
内置了一个关键原则：一点点微小的改进如果引入了丑陋的复杂性，不值得保留；但在保持性能的同时删除代码，永远是一个胜利。这防止了代码无限膨胀。
...
```
1234567891011# 1. 安装 uvcurl -LsSf https://astral.sh/uv/install.sh | sh # 2. 安装依赖uv sync # 3. 下载数据 + 训练分词器（一次性,约 2 分钟）uv run prepare.py # 4. 手动跑一次训练验证环境uv run train.py
```

环境跑通后，把你的 AI 编码智能体（Claude Code、Codex CLI 等）指向 
...
autoresearch 不是要取代研究者，而是改变了研究者的工作内容。你不再是那个在 
