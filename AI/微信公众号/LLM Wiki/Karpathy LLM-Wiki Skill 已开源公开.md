> 📎 来源: [四分地](https://mp.weixin.qq.com/s?__biz=MzIzMjE4NjM1NA==&mid=2649010596&idx=1&sn=d405e50e9c8773d787b123344654fd39&chksm=f1615cee8aabae02845393a9541552fa9fddef2f42c828d92cc863e1a5ddc3e1997bbdb9e0fa&mpshare=1&scene=1&srcid=0511wKYSqj83sTgAbcwinUn9&sharer_shareinfo=9fffa368abfa3609e31963ecff912465&sharer_shareinfo_first=9fffa368abfa3609e31963ecff912465) | 时间: 2026-05-11 00:04

---

一个可安装的 Skill，加上一个真实运行中的参考实现，用来构建由 LLM 持续维护的 Markdown Wiki。

![](assets/img_a27fbe62b282.png)
...
这个示例默认你使用的是 OpenAI Codex，所以生成的 schema 文件会是

```
AGENTS.md
```

。如果你选择的是 Claude Code，把同一步里的
...
3. 3. 等 Wiki 脚手架生成后，把这份原始理念文档复制到新 Wiki 的
...
```
llm-wiki/├── AGENTS.md├── raw/│   ├── Karpathy x.md│   └── llm-wiki-pattern.md└── wiki/    ├── index.md    ├── log.md    ├── overview.md    ├── concepts/    ├── entities/    ├── comparisons/    ├── sources/    └── synthesis/
```
...
```
.agent/└── skills/    └── llm-wiki-bootstrap/        ├── SKILL.md        └── references/
```
...
是一个可安装的 Skill，用来创建由 LLM 持续维护的 Markdown Wiki，同时附带一个真实的

```
llm-wiki/
```

参考实现，展示这套模式如何以 Karpathy 的原始 LLM Wiki 思路为起点真正运行起来。
