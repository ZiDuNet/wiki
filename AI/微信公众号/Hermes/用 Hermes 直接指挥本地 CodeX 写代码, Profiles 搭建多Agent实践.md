...
```
coder setup         # 配置 API 密钥和模型coder chat          # 开始对话coder gateway start # 启动 Gateway
```
...
```
hermes profile list              # 查看所有 Profile 状态hermes profile show coder        # 查看 coder 的详细配置hermes profile export coder      # 导出备份为 coder.tar.gzhermes profile rename coder dev  # 重命名，同步更新别名和服务hermes update                    # 更新代码并同步所有 Profile 的内置技能
```
