...
```
created → ready → running → completed                  ↓                blocked → (unblock) → ready                  ↓                crashed/timed_out → ready (重试)                  ↓              archived (最终状态)
```
...
```
父任务：实现用户认证系统├── T1 (researcher): 调研 OAuth2 和 JWT 方案├── T2 (backend-eng): 实现认证中间件和 API (依赖 T1)└── T3 (qa-engineer): 编写单元测试和集成测试 (依赖 T2)
```
