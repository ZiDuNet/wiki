> 📎 来源: [单向箔](https://mp.weixin.qq.com/s?__biz=Mzg4MDY0Mzg3OQ==&mid=2247483891&idx=1&sn=03322251430564a48c2be1e084bf7146&chksm=cec84b1c55ea2460dd04397aeaa0f2a9a7ea631bc957d56afa74402dffabde815bb9c984b72e&mpshare=1&scene=1&srcid=0513MDrKVJg4H3IL1Ti6OcAj&sharer_shareinfo=39b4499d5ea891f9cd814dccfd5eb2db&sharer_shareinfo_first=39b4499d5ea891f9cd814dccfd5eb2db) | 时间: 2026-05-13 15:39
...
 文件的文件夹，里面写着指令或脚本。AI 读到它，就能多干一件事。
...
- ```
  ...
  ——布局实验性（1-3: 简洁居中 / 8-10: 非对称现代）
- ```
  MOTION_INTENSITY
  ```

  ——动效强度（1-3: 简单悬停 / 8-10: 磁性 + 滚动触发）
- ```
  VISUAL_DENSITY
  ```

  ——视觉密度（1-3: 宽松奢华 / 8-10: 信息密集仪表板）

这个设计很聪明。同一个 Skill，做博客主页把 
...
 调到 8，出来的风格完全不同（终于不用每次都手动给 AI 描述「我想要现代感的设计」了）。
...
 CLI 让 AI 直接操作 GitHub——查 PR、看 CI 跑了几条、看哪个 Step 挂了、用 API 拿到你想要的数据、翻 issue 列表……全在对话窗口里完成，不用切浏览器。
...
它让 AI 可以直接操作你的 vault：搜索笔记全文、创建新笔记、移动和重命名、删除。所有操作通过 
